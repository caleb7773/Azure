...
echo -e " ###  ${GREEN}Endpoint Protocol:${RED} ${PROTOCOL}${NC}"
echo " ###"
echo " #############################################################################"
echo " #############################################################################"
echo
...
az tag create --resource-id $grouppy --tags UserAzure=${username} AzureDeployingUser=${username} Persistent=unknown UseCase=unknown Scripted=TechAzurePanda > /dev/null
echo -e "${NC}"
...
echo -e "    if you need to limit exposure to only your back end Public IP, it is: ${GREEN}${VM_2_Public_IP_ADDRESS}${NC}"
echo
